...
def find_silver_token():
    ...
    dist=100
    for token in R.see():
        if token.dist < dist and token.info.marker_type is MARKER_TOKEN_SILVER and token.info.code not in arr_silver:
            dist=token.dist
	    rot_y=token.rot_y
...
def find_golden_token():
    ...
    dist=100
    for token in R.see():
        if token.dist < dist and token.info.marker_type is MARKER_TOKEN_GOLD and token.info.code not in arr_golden :
            dist=token.dist
	    rot_y=token.rot_y
	    box_id = token.info.code
    if dist==100:
...
while 1:
	...
	drive(10,1)
        ...
        if hold: # token is grabbed by robot, then release token
            drive(10,2)
            arr_golden.append(box_id) # token ID is saved in array of boxes where silver box is released
            R.release()
            drive(-20, 2)
